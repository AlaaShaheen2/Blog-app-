...
RSpec.describe 'User show Page', type: :feature do
  before :each do
    @user = User.create(name: 'Alaa', photo: 'photo.png', bio: 'Developer', postCounter: 12)
    @post = Post.create(title: 'Ruby', text: 'Ruby is easy', comments_counter: 7, likes_counter: 6,
                        author: @user)
  end
  ...
  describe 'User show page' do
    ...
    it 'show a container for the users' do
      expect(page).to have_css('section.a_user_container')
    end
    ...
    it 'shows the number of posts the user has posted' do
      expect(page).to have_content("Number of posts: #{@user.postCounter || 0}")
    end
